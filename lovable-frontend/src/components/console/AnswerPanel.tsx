...
const EXAMPLES = [
  "Explain the architecture of a multi-agent RAG system with hierarchical routing.",
...
export function AnswerPanel({ status, result, onPickExample }: AnswerPanelProps) {
      ...
      <div className="flex flex-1 flex-col overflow-y-visible p-6 sm:p-8 xl:overflow-y-auto xl:border-r xl:border-border">
        <div className="m-auto flex max-w-md flex-col items-center text-center">
          ...
          <p className="mt-2 text-sm leading-relaxed text-foreground-subtle">
            The agent will route your question, retrieve repository context, grade its relevance,
            and generate an answer with a full execution trace.
          </p>
  ...
  return (
    <div className="flex-1 overflow-y-visible p-6 sm:p-8 xl:overflow-y-auto xl:border-r xl:border-border">
      <div className="mx-auto max-w-[56ch] animate-fade-in">
        <div className="mb-6 flex flex-wrap items-center gap-4">
          <span className="rounded border border-border-strong bg-card px-2 py-0.5 font-mono text-xs text-foreground-subtle">
        ...
        <h1 className="mb-8 text-balance text-2xl font-medium italic leading-tight text-foreground-strong sm:text-3xl">
          {answer.headline}
        </h1>

        <div className="space-y-6 text-pretty text-foreground">
          ...
          {answer.bullets ? (
            <ul className="space-y-4 border-l border-border-strong pl-6 pt-4">
              {answer.bullets.map((bullet) => (
                <li key={bullet.title} className="text-sm">
                  <span className="mb-1 block font-medium text-foreground-strong">
                    {bullet.title}
                  </span>
        ...
        <div className="mt-12 flex flex-wrap justify-between gap-6 border-t border-border py-4">
